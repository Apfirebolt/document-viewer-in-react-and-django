...
class PublicUserApiTests(TestCase):
    ...
    def test_update_username(self):
        """Test updating the username for the authenticated user."""
        user = create_user(
            email="test@example.com", password="testpass123", username="Old Name"
        )
        self.client.force_authenticate(user=user)

        payload = {"username": "New Name"}
        res = self.client.put(detail_user_url(user.id), payload)
